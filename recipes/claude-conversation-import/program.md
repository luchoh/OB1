...
- Do not change `import-claude.py`, `eval-prompt.py`, or `eval-cases.json` during the tuning loop.
...
Loop:
1. Run the baseline:
   - `../chatgpt-conversation-import/.venv/bin/python -u eval-prompt.py <claude-export.zip> --report prompt-eval-report.json`
