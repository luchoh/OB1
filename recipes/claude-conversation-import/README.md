# Claude Conversation Import

Import a Claude data export into Open Brain as distilled, searchable thoughts.

## What It Does

- reads a Claude export zip or extracted folder
- targets the current Anthropic export shape centered on `conversations.json`
- tolerates small schema differences around conversation and message field names
- preserves attachment filenames as text context when they are present in the export
- filters low-value conversations
- distills each kept conversation into a selective set of durable thoughts
- uses an adaptive thought cap: usually `1-3`, but up to `7` for dense conversations
- applies deterministic secret hygiene before ingest, redacting obvious literal secrets from both extracted thoughts and stored source text
- ingests those thoughts into the local OB1 service

The local-first path uses:
- local Qwen inference through the `mlx-server` Consul service
- local OB1 ingest on `localhost:8787`
- local embeddings through the OB1 service
- no internet egress in steady state
- Qwen tool calling for structured extraction

The prompt is now intentionally externalized in [prompt.md](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/prompt.md#L1), so it can be tuned without rewriting the importer.

## Prerequisites

- Python 3.10+
- `requests` installed from [requirements.txt](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/requirements.txt)
- the local OB1 service running
- your real `.env.open-brain-local`
- a Claude export zip or extracted folder

## Export Your Claude History

Anthropic currently supports exporting Claude data from:
- Claude web / Claude Desktop: `Settings -> Privacy -> Export data`
- Team workspaces: Primary Owner only
- Enterprise workspaces: `Settings -> Data management -> Export Data`

The email download link expires after 24 hours.

Official reference:
- https://support.anthropic.com/en/articles/9450526-how-can-i-export-my-claude-data

The importer accepts either:
- the downloaded zip directly
- the extracted folder

Current boundary:
- the importer reads conversation JSON plus message-level attachment metadata
- it does not separately ingest binary exported files or images
- if Anthropic changes the export format materially, run a dry-run first and we can tighten the parser against the real export

## Local Run

From the repo root:

```bash
cd recipes/claude-conversation-import
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
set -a
source ../../.env.open-brain-local
set +a
python import-claude.py /path/to/claude-export.zip --dry-run --limit 10
```

If the dry run looks right:

```bash
python import-claude.py /path/to/claude-export.zip
```

This importer uses Qwen tool calling for thought extraction rather than `response_format`, because that is the reliable structured-output path on the current `mlx-server`. Details are in [docs/08-vllm-mlx-no-thinking.md](/Users/luchoh/Dev/OB1/docs/08-vllm-mlx-no-thinking.md#L1).

## Important Flags

- `--dry-run`: parse, filter, and summarize without writing
- `--limit N`: test with a small subset first
- `--after YYYY-MM-DD`: only import newer conversations
- `--before YYYY-MM-DD`: only import older conversations
- `--raw`: skip summarization and ingest the user text directly
  - secret hygiene still applies before ingest
- `--verbose`: print the full extracted thoughts
- `--report FILE`: write a markdown import report

## Runtime Defaults

By default the script uses:

- summarizer model: `LLM_MODEL`
- summarizer endpoint: `LLM_BASE_URL` or Consul discovery of `mlx-server`
- summarizer thinking mode: `LLM_ENABLE_THINKING=false`
- ingest endpoint: `http://localhost:8787/ingest/thought`
- ingest key: `MCP_ACCESS_KEY`

You can override the ingest path with:

```bash
export OPEN_BRAIN_INGEST_URL=http://localhost:8787/ingest/thought
export OPEN_BRAIN_INGEST_KEY="$MCP_ACCESS_KEY"
```

## Optional Local Fallback

If you want to summarize with Ollama instead of the canonical MLX endpoint:

```bash
python import-claude.py /path/to/claude-export.zip --model ollama --ollama-model qwen3
```

## Troubleshooting

`No Claude conversations JSON files found`
- Point the script at the export zip or the extracted export directory.

`Unauthorized`
- The local OB1 service is up, but the importer does not have the right `MCP_ACCESS_KEY`.

`No thoughts extracted`
- This is expected for low-value or purely transactional conversations. Use `--raw` if you want everything.

`Already imported`
- The sync log is `claude-sync-log.json` in this recipe directory. Remove it if you want to re-run from scratch.

`Why did one conversation produce more than 3 thoughts?`
- The importer uses an adaptive cap based on conversation size. Long dense conversations can yield more than `3`, but the model is still instructed to stay selective and return fewer when appropriate.

## Prompt QA

This recipe now has an autoresearch-style prompt-evaluation harness:

- mutable artifact: [prompt.md](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/prompt.md#L1)
- fixed evaluator: [eval-prompt.py](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/eval-prompt.py#L1)
- directed case set: [eval-cases.json](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/eval-cases.json#L1)
- optimizer loop: [optimize-prompt.py](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/optimize-prompt.py#L1)

Run it like this:

```bash
cd recipes/claude-conversation-import
set -a
source ../../.env.open-brain-local
set +a
python eval-prompt.py /path/to/claude-export.zip --report prompt-eval-report.json
```

The intended workflow is:
- change only `prompt.md`
- re-run `eval-prompt.py`
- keep prompt revisions only when the directed QA score improves

For a concise autoresearch-style task definition, see [program.md](/Users/luchoh/Dev/OB1/recipes/claude-conversation-import/program.md#L1).
